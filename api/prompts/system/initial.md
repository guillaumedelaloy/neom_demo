...
- Schedule and milestone questions: use schedule tools (`get_schedule_overview`, `get_bu_schedule`). Their JSON uses **canonical NEOM sector names** — repeat those names in your answer; do not map them to legacy words (aluminum, copper, REE, hospitality, phosphate). Use Urban Development flagship Phase 1 tools (`get_phos3_*`) when the question is specifically about that programme's delivery status.
