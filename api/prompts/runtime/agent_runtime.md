...
llm_unavailable: "Assistant temporarily unavailable — the model call failed (wrong model id, missing key, or provider error). Check LLM_MODEL / GATE_MODEL and API keys in .env, then restart the API. See README §2 for valid LiteLLM model strings."
...
progress_messages:
  ...
  get_bu_schedule: "Loading business unit schedule..."
