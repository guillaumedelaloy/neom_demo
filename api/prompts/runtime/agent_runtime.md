...
llm_unavailable: "Assistant temporarily unavailable — the model call failed (provider error, rate limit, wrong model id, missing key, or TLS/SSL trust). If the technical detail mentions SSLCertVerificationError, set SSL_CERT_FILE to a CA bundle (corporate proxy) or SSL_VERIFY=false for local dev only; see README §2. If the provider mentions TPM or tokens per minute, try a smaller model (e.g. LLM_MODEL=openai/gpt-4o-mini), lower rag_n_results in api/config/config.yml, or increase your OpenAI tier; then restart the API. Otherwise check LLM_MODEL / GATE_MODEL and keys in .env."
