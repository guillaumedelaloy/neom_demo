# NEOM Strategy Cockpit — UI look & feel (reference)

Use this document to align prototypes with the app’s **NEOM** visual language. Prefer **`ma-accent`** (primary highlight) and **`ma-teal`** (digital / positive).
...
- **Brand accents** — **teal** for primary actions, positive / “on track” signals, and links; **accent** (white on dark / black on light) for emphasis and active nav affordances; **charcoal** for strong primary buttons and nav depth.
...
| `--ma-accent` | `#b8956a` | Accent, active nav icon |
| `--ma-accent-muted` | `#8f7354` | Muted accent metrics |
...
ma: {
  ...
  'accent-muted': 'var(--ma-accent-muted)',
  teal: 'var(--ma-teal)',
...
- **Nav items:** `rounded-sm`, `px-3 py-3`, `gap-3`. **Active:** `border-[color:var(--ma-accent)]/50`, `bg-white/[0.08]`, icon `text-[color:var(--ma-accent)]`. **Inactive:** muted text, hover `hover:bg-white/[0.04]` / `hover:text-white`.
...
- **Secondary:** `bg-ma-elevated text-ma-ink border-ma-line hover:border-ma-accent/50`.
...
- **Tones (light app):** neutral on `ma-surface`; teal tint `#e8f2f1` / border `#c5dedb`; soft warm neutral tint `#f3ebe0` / border `#e5d5c4`; risk tint `#f6e8e8` / border `#e8cfcf`; muted on `ma-bg`.
...
- **Semantic colors in charts:** teal = positive band; accent stroke for emphasis; amber dashed = benchmark / warning line; red / risk for merit-order envelope where used.
...
- Storage key pattern: e.g. `neom-theme` with values `light` | `dark`.
...
*Generated from the NEOM Strategy Cockpit prototype UI system.*
